# Page 02 — About Us (`pages/about-us.js`)

## Purpose

The About Us page must tell a compelling, credible story — not generic template text. Government tender committees, NGO procurement officers, and contractors actively read About pages to assess:

- Company background and legitimacy
- Leadership and expertise
- Sector focus and specialisation
- Mission and long-term vision
- Why this company is specifically suited for canal/irrigation/rural works

---

## Section Breakdown

### Section 1 — Page Hero

**Layout:** Full-width banner with site/project photo background and dark overlay.

**Headline:**
```
About Wave Earth Infratech
```

**Subheadline:**
```
Infrastructure Execution Specialists — Canal, Irrigation & Rural Development
```

**Breadcrumb:** `Home / About Us`

---

### Section 2 — Company Introduction

**Heading:** `Who We Are`

**Content (write in company's own words; example framework below):**

> Wave Earth Infratech is a civil infrastructure execution company specialising in canal construction, nahar lining, irrigation works, and rural redevelopment projects across Uttar Pradesh and Madhya Pradesh.
>
> Founded with a mission to bring professional infrastructure execution to India's underserved rural zones, we work closely with state government irrigation departments, Jal Shakti Ministry projects, and development-sector NGOs to deliver durable, impactful civil works.
>
> Our team has direct field experience across [X] years in earthwork, concrete structures, water channel development, and large-scale civil maintenance contracts.

**Layout:** 2-column — text left, strong site photo right.

**Key facts strip below introduction:**

| Fact | Value |
|---|---|
| Year Established | [Year] |
| Registered Office | Chitrakoot, UP |
| Work Zones | Uttar Pradesh, Madhya Pradesh |
| Primary Sector | Irrigation, Canal, Rural Infrastructure |
| MSME Registration | Yes |
| GST Registered | Yes |

---

### Section 3 — Mission & Vision

**Layout:** Side-by-side cards or full-width alternating blocks.

**Mission card:**
```
Mission
To deliver high-quality, timely, and cost-effective civil infrastructure solutions
that improve water access, agricultural productivity, and rural livelihoods across
Central India.
```

**Vision card:**
```
Vision
To become the most trusted infrastructure execution partner for government
departments and development organisations working in canal and irrigation
development across UP and MP.
```

---

### Section 4 — Our Story

**Heading:** `Our Journey`

**Timeline layout (vertical or horizontal):**

| Year | Milestone |
|---|---|
| [Year] | Company founded in Chitrakoot |
| [Year] | First government irrigation contract awarded |
| [Year] | Expanded to MP; first Jal Shakti project |
| [Year] | MSME registration completed |
| [Year] | Crossed [X] km of canal works delivered |
| [Year] | First NGO partnership for rural development |
| Present | Active execution across [X] project sites |

**Note:** Fill in real years and milestones. Even a short genuine history is more credible than a fabricated story.

---

### Section 5 — Founder / Director Section

**Heading:** `Leadership`

**Layout:** Card with photo, name, designation, and short bio.

**For each leader (Director / MD / Project Head):**

- Professional photograph (not casual, not stock)
- Name
- Designation
- Brief bio (3–4 lines):
  - Years of experience
  - Sector expertise
  - Key past achievements
  - Educational background (if relevant)

**Example:**
> Mr. [Name], Director
> With over [X] years of hands-on experience in canal and civil infrastructure projects,
> [Name] leads Wave Earth Infratech's field operations and government contract execution.
> He has overseen projects worth ₹[X] crore across UP and MP for state irrigation
> departments and central government schemes.

---

### Section 6 — Core Values

**Heading:** `What Drives Us`

**Layout:** 4–6 cards with icon.

| Value | Description |
|---|---|
| Integrity | Honest reporting, transparent billing, and reliable communication with clients |
| Quality | No compromise on material specification or workmanship standards |
| Timeliness | We take deadline commitments seriously — no exception |
| Safety | Site safety protocols enforced on all worksites |
| Rural Focus | Dedicated to improving the quality of life in rural and semi-urban India |
| Sustainability | Water conservation and eco-sensitive execution practices |

---

### Section 7 — Sector Expertise

**Heading:** `Our Areas of Expertise`

**Layout:** Icon list or horizontal pills.

- Canal Construction & Lining
- Irrigation Infrastructure Development
- Nahar Cleaning & Desilting
- Earthwork & Bulk Excavation
- RCC Structures & Retaining Walls
- Road & Drainage Works
- Water Resource Development
- Rural Redevelopment Projects
- Government Scheme Execution (PMGSY, Jal Jeevan, PMKSY etc.)
- NGO Civil Works

---

### Section 8 — Why We Are Different

**Heading:** `Infrastructure Execution — the Way It Should Be Done`

**Content (2–3 paragraphs or bullet points):**

Key differentiators to highlight:
- Own machinery fleet (no dependency on rentals for critical equipment)
- Locally embedded team with knowledge of UP/MP geography and conditions
- Government compliance experience — proper billing, measurement books, inspection readiness
- Ability to mobilise quickly for urgent government or NGO contracts
- Experience working in challenging terrain (flood-prone, seasonal, remote sites)

---

### Section 9 — Certifications Summary

Show badges: MSME, GST, PAN, Contractor License, ISO (if applicable).

**CTA:** `View All Documents →` links to `/certifications`

---

### Section 10 — CTA Band

```
Ready to partner with us on your next infrastructure project?
```

Buttons:
- `Contact Us`
- `Download Company Profile`

---

## SEO Metadata

```
Title:    About Wave Earth Infratech | Canal & Irrigation Contractor Chitrakoot UP
Description: Learn about Wave Earth Infratech — a civil infrastructure execution company
             specialising in canal construction, nahar lining, irrigation works, and rural
             redevelopment projects for government departments and NGOs across UP and MP.
Keywords: about Wave Earth Infratech, canal contractor UP, infrastructure company Chitrakoot,
          irrigation contractor Madhya Pradesh, government civil contractor India
```

---

## Content Checklist

- [ ] Company introduction text (genuine, in own words)
- [ ] Real founding year and history milestones
- [ ] Director/founder professional photo
- [ ] Director bio written
- [ ] Mission and vision statements approved
- [ ] Core values confirmed
- [ ] Sector expertise list verified
- [ ] Certification badges ready
- [ ] Company profile PDF linked
