# Page 03 - Services (`pages/services.js`)

## Purpose

The Services page must replace the current generic "construction services" listing with government-sector-specific, technically detailed service descriptions. Each service should clearly explain the scope, process, machinery involved, and typical government/NGO use case.

Government evaluators use this page to assess whether the company can handle a specific type of tender.

---

## Section Breakdown

### Section 1 - Page Hero

**Headline:** `Our Services`
**Subheadline:** `Specialised Civil Infrastructure Services for Government Departments, Irrigation Bodies & NGOs`
**Breadcrumb:** `Home / Services`

---

### Section 2 - Services Grid Overview

**Layout:** Top-of-page grid showing all services as cards with icon. Each card links down to its detailed section or to a dedicated sub-page.

| # | Service Name | Route (optional sub-page) |
|---|---|---|
| 1 | Canal Construction & Repair | `/services/canal-construction` |
| 2 | Nahar Lining Works | `/services/nahar-lining` |
| 3 | Irrigation Infrastructure | `/services/irrigation-infrastructure` |
| 4 | Rural Redevelopment | `/services/rural-redevelopment` |
| 5 | Water Resource Projects | `/services/water-resource-projects` |
| 6 | Earthwork & Excavation | `/services/earthwork-excavation` |
| 7 | Concrete Structures | `/services/concrete-structures` |
| 8 | Road & Drainage Works | `/services/road-drainage` |
| 9 | NGO Development Projects | `/services/ngo-development` |
| 10 | Civil Maintenance Contracts | `/services/civil-maintenance` |

**Note:** Sub-pages are strongly recommended for SEO. Each service sub-page can rank for a specific keyword (e.g., "Nahar Lining Contractor UP").

---

## Individual Service Specifications

Each service block/sub-page must contain the following structure:

---

### Service 1 - Canal Construction & Repair

**Photo:** Active canal construction site. JCB or excavator working at canal edge.

**Description:**
> We execute new canal construction and canal repair works for state irrigation departments, including clearing, desilting, bank reinforcement, and structural repair of deteriorated canal sections. Our work is aligned with PMKSY and state Jal Shakti scheme requirements.

**Work Scope:**
- New canal excavation (earthen and pucca)
- Canal bank earthwork and compaction
- Desilting and deepening of existing canals
- Stone pitching and slope protection
- Head regulator and cross structures
- Canal rejuvenation under government schemes

**Work Process:**
1. Site survey and alignment verification
2. Mobilisation of excavators and earthmoving equipment
3. Excavation to specified dimensions
4. Bank formation and compaction
5. Stone pitching / RCC lining as specified
6. Final profile check and handover

**Machinery Used:**
- Excavator (1.5m³ bucket)
- JCB 3DX
- Tipper trucks
- Compaction rollers
- Water tankers for compaction

**Typical Client:** State Irrigation Department, Jal Shakti, PMKSY executing agencies

---

### Service 2 - Nahar Lining Works

**Photo:** Workers laying concrete canal lining. Close-up of lining work in progress.

**Description:**
> Nahar lining prevents water seepage losses and extends canal lifespan by 3-5 times. We execute cement concrete and HDPE lining works for minor and major irrigation channels as per BIS/IS standards and departmental specifications.

**Work Scope:**
- Cement concrete canal lining (M20/M25)
- Pre-cast concrete lining panels
- HDPE/Geo-membrane lining
- Side slope and bed preparation
- Expansion joints and sealing
- Lining inspection and quality testing

**Work Process:**
1. Canal bed and side slope profiling
2. Sub-grade preparation and compaction
3. Formwork placement
4. Concrete pouring and vibration
5. Curing (minimum 21 days)
6. Joint sealing and final inspection

**Machinery Used:**
- Concrete mixer (500L / 1000L)
- Slipform paver (if applicable)
- Plate compactors
- Concrete vibrators
- Transit mixer (for large volumes)

**Typical Client:** Minor Irrigation Department, Canal Officer, Jal Nigam

---

### Service 3 - Irrigation Infrastructure

**Photo:** Overhead or drone view of irrigation network with farmland visible.

**Description:**
> Complete irrigation infrastructure development including distribution channels, field channels, water control structures, and last-mile connectivity works ensuring water reaches agricultural fields efficiently.

**Work Scope:**
- Distribution channel construction
- Field channel (rajwaha) works
- Water control structures (escapes, falls, regulators)
- Farm water outlet construction
- Lift irrigation pump house construction
- Pipe irrigation works

**Typical Client:** Agriculture Department, Jal Shakti, Rural Development bodies

---

### Service 4 - Rural Redevelopment

**Photo:** Village road or community infrastructure construction.

**Description:**
> Comprehensive rural infrastructure improvement works including internal roads, drainage systems, community structures, and basic civic amenity construction under government and NGO-funded schemes.

**Work Scope:**
- Village internal roads (CC/WBM/Gravel)
- Storm water drains and culverts
- Community hall / panchayat bhawan construction
- School/Anganwadi infrastructure
- Public toilet blocks
- Boundary walls and fencing

**Typical Client:** Panchayati Raj Department, DRDA, NGOs, CSR partners

---

### Service 5 - Water Resource Projects

**Photo:** Checkdam, pond recharge structure, or water storage tank.

**Description:**
> Construction of water harvesting and storage structures including checkdams, ponds, percolation tanks, and overhead water tanks to improve water availability in water-stressed rural areas.

**Work Scope:**
- Checkdam construction (masonry / RCC)
- Farm pond construction
- Percolation tank construction
- Overhead water storage tanks
- Underground sump construction
- Recharge shaft construction

**Typical Client:** Groundwater Department, MNREGS executing agencies, NGOs (water & sanitation)

---

### Service 6 - Earthwork & Excavation

**Photo:** Large-scale earthmoving site. Excavator dumping soil.

**Description:**
> Heavy earthwork and bulk excavation services for civil and infrastructure projects including site clearing, embankment formation, cut-and-fill operations, and subgrade preparation.

**Work Scope:**
- Bulk excavation (rock and soil)
- Embankment/bund formation
- Filling and compaction
- Sub-grade preparation for roads
- Topsoil stripping and stockpiling
- Slope cutting and shaping

**Machinery Used:**
- Excavators (multiple sizes)
- JCB
- Bulldozer / Dozer (if applicable)
- Tipper trucks (10-20 tonne)
- Motor Grader
- Vibratory roller

---

### Service 7 - Concrete Structures

**Photo:** RCC retaining wall or concrete slab being poured.

**Description:**
> Design-and-build or drawing-based RCC structure construction for civil and infrastructure projects including retaining walls, slabs, columns, beams, foundations, and water-retaining structures.

**Work Scope:**
- RCC retaining walls
- Foundation work (isolated, strip, raft)
- Columns, beams, slabs
- RCC water tanks and sumps
- Bridge culverts and causeways
- Canal cross-drainage structures

---

### Service 8 - Road & Drainage Works

**Photo:** Village road construction or drain excavation.

**Description:**
> Construction of rural access roads, link roads, and drainage systems connecting villages to main roads and ensuring stormwater management for rural and peri-urban communities.

**Work Scope:**
- Gravel / WBM / CC roads
- Bituminous (BT) road works (surface dressing)
- Kerb and channel construction
- Storm water drains (brick/RCC)
- Culverts and causeways
- Road marking and signage

**Typical Client:** PWD, PMGSY, Urban Local Bodies, Gram Panchayat

---

### Service 9 - NGO Development Projects

**Photo:** Field workers with NGO/community members at a project site.

**Description:**
> Dedicated civil execution capability for development-sector projects funded by NGOs, foundations, CSR bodies, and bilateral aid programmes. We understand the reporting, documentation, and community engagement expectations of the development sector.

**Work Scope:**
- Community water infrastructure
- School / health centre construction
- Sanitation and WASH infrastructure
- Livelihood-linked civil works
- Geo-technical community structures

**Typical Client:** National and international NGOs, foundations, CSR teams

---

### Service 10 - Civil Maintenance Contracts

**Photo:** Maintenance crew working on canal or road.

**Description:**
> Annual Maintenance Contracts (AMC) and Operation & Maintenance (O&M) services for government-owned infrastructure assets including canals, roads, and water structures. Regular inspection, repair, and reporting as per department standards.

**Work Scope:**
- Canal cleaning and desilting (annual)
- Canal bank maintenance
- Road pothole repair and patch work
- Structure inspection and reporting
- Emergency repair mobilisation
- Vegetation removal from canal beds

---

## SEO Metadata

```
Title:    Services | Canal Construction, Nahar Lining & Irrigation Works - Wave Earth Infratech
Description: Wave Earth Infratech offers canal construction, nahar lining, irrigation
             infrastructure, earthwork, concrete structures, road works, and NGO project
             execution across UP and MP for government and development sector clients.
Keywords: canal construction contractor, nahar lining contractor UP, irrigation infrastructure
          contractor, rural redevelopment contractor India, earthwork contractor UP MP,
          NGO civil contractor India
```

---

## Content Checklist

- [ ] One real photo per service (from actual project sites)
- [ ] Service descriptions reviewed and approved by team
- [ ] Machinery list confirmed with fleet inventory
- [ ] Typical client types verified
- [ ] Sub-page routes decided (single page with anchors vs separate pages)
- [ ] Each service description written with real project experience woven in
