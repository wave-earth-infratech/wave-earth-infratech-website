...
export async function getStaticProps() {
  ...
  const projects = all.filter((p) => p.status === 'ongoing')
  return { props: { projects, total: all.length } }
}

export default function OngoingProjectsPage({ projects, total }) {
  return (
    <>
      <Head>
        <title>Ongoing Projects | Active Infrastructure Works - Wave Earth Infratech</title>
        <meta name="description" content="Active canal construction, irrigation, earthwork, and infrastructure projects currently under execution by Wave Earth Infratech in Uttar Pradesh and Madhya Pradesh." />
      </Head>
      ...
      <main className="bg-theme-base text-theme-fg min-h-screen">
        ...
        <section className="pt-36 pb-16 bg-theme-surface border-b border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-theme-fg-3 font-sans mb-8" aria-label="Breadcrumb">
              ...
              <span>/</span>
              <span className="text-[#52B788]">Ongoing</span>
            </nav>
            <div className="flex items-center gap-3 mb-5">
              <span className="w-8 h-px bg-amber-500" />
              <span className="text-amber-400 text-[13px] uppercase tracking-[0.2em] font-sans font-semibold">Active</span>
            </div>
            ...
            <p className="text-theme-fg-2 text-lg max-w-2xl leading-relaxed font-sans">
              Active construction sites currently under execution - real-time delivery across UP &amp; MP.
            </p>
          </div>
        </section>
        ...
        <div className="border-b border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-wrap gap-10 items-center">
            {[
              [projects.length.toString(), 'Active Sites'],
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {projects.length === 0 ? (
              <div className="py-24 text-center text-theme-fg-3 font-sans text-sm uppercase tracking-widest">
                No ongoing projects at present. <Link href="/projects" className="text-[#52B788] hover:underline">View all projects →</Link>
              </div>
            ) : (
                  ...
                  <div key={project.slug} className="group bg-theme-base hover:bg-theme-card transition-colors duration-300 flex flex-col">
                    <div className="relative overflow-hidden aspect-[16/9]">
                      ...
                      <div className="absolute top-4 right-4">
                        <span className="bg-amber-500/20 text-amber-400 border border-amber-500/40 text-[10px] font-bold uppercase tracking-widest px-3 py-1.5">
                          Ongoing
                        </span>
                      </div>
                    </div>
