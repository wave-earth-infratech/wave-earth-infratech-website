...
export async function getStaticProps() {
  ...
  const projects = all.filter((p) => p.category === 'rural-redevelopment')
  return { props: { projects, total: all.length } }
}

export default function RedevelopmentProjectsPage({ projects, total }) {
  return (
    <>
      <Head>
        <title>Redevelopment Works | Rural Infrastructure Projects - Wave Earth Infratech</title>
        <meta name="description" content="Rural redevelopment and community infrastructure uplift projects executed by Wave Earth Infratech across Uttar Pradesh and Madhya Pradesh." />
      </Head>
      ...
      <main className="bg-theme-base text-theme-fg min-h-screen">
        ...
        <section className="pt-36 pb-16 bg-theme-surface border-b border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-theme-fg-3 font-sans mb-8" aria-label="Breadcrumb">
              ...
              <span>/</span>
              <span className="text-[#52B788]">Redevelopment Works</span>
            </nav>
            <div className="flex items-center gap-3 mb-5">
              <span className="w-8 h-px bg-[#52B788]" />
              <span className="text-[#52B788] text-[13px] uppercase tracking-[0.2em] font-sans font-semibold">Rural</span>
            </div>
            ...
            <p className="text-theme-fg-2 text-lg max-w-2xl leading-relaxed font-sans">
              Rural and community infrastructure uplift - village roads, civic structures &amp; public works across UP &amp; MP.
            </p>
          </div>
        </section>
        ...
        <div className="border-b border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-wrap gap-10 items-center">
            {[
              [projects.length.toString(), 'Redevelopment Projects'],
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {projects.length === 0 ? (
              <div className="py-24 text-center text-theme-fg-3 font-sans text-sm uppercase tracking-widest">
                No redevelopment projects found. <Link href="/projects" className="text-[#52B788] hover:underline">View all projects →</Link>
              </div>
            ) : (
        ...
        <section className="py-16 bg-theme-surface border-t border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <h3 className="font-display text-[28px] text-theme-fg uppercase">Rural Redevelopment</h3>
            </div>
            <div className="flex gap-4">
              <Link href="/services/rural-redevelopment" className="btn-primary">View Service Details</Link>
