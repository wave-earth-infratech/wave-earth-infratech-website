...
export async function getStaticProps() {
  ...
  const projects = all.filter((p) => p.category === 'canal-construction')
  return { props: { projects, total: all.length } }
}

export default function CanalProjectsPage({ projects, total }) {
  return (
    <>
      <Head>
        <title>Canal Projects | Canal Construction & Lining Works - Wave Earth Infratech</title>
        <meta name="description" content="Canal construction, lining, desilting and irrigation channel works executed by Wave Earth Infratech across Uttar Pradesh and Madhya Pradesh." />
      </Head>
      ...
      <main className="bg-theme-base text-theme-fg min-h-screen">
        ...
        <section className="pt-36 pb-16 bg-theme-surface border-b border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-theme-fg-3 font-sans mb-8" aria-label="Breadcrumb">
              ...
              <span>/</span>
              <span className="text-[#52B788]">Canal Projects</span>
            </nav>
            <div className="flex items-center gap-3 mb-5">
              <span className="w-8 h-px bg-[#52B788]" />
              <span className="text-[#52B788] text-[13px] uppercase tracking-[0.2em] font-sans font-semibold">Canal Works</span>
            </div>
            ...
            <p className="text-theme-fg-2 text-lg max-w-2xl leading-relaxed font-sans">
              Canal construction, concrete lining, desilting &amp; bank reinforcement for state irrigation departments.
            </p>
          </div>
        </section>
        ...
        <div className="border-b border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-wrap gap-10 items-center">
            {[
              [projects.length.toString(), 'Canal Projects'],
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {projects.length === 0 ? (
              <div className="py-24 text-center text-theme-fg-3 font-sans text-sm uppercase tracking-widest">
                No canal projects found. <Link href="/projects" className="text-[#52B788] hover:underline">View all projects →</Link>
              </div>
            ) : (
        ...
        <section className="py-16 bg-theme-surface border-t border-theme-border/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <h3 className="font-display text-[28px] text-theme-fg uppercase">Canal Construction & Repair</h3>
            </div>
            <div className="flex gap-4">
              <Link href="/services/canal-construction" className="btn-primary">View Service Details</Link>
